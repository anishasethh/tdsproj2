...
async def transcribe_youtube_segment(youtube_url: str, start_time: float, end_time: float) -> str:
    ...
    Returns:
        str: The transcribed text from the segment.
    """
    try:
        ...
        # Step 2: Load audio and extract the segment
        ...
        with tempfile.NamedTemporaryFile(suffix='.wav', delete=False) as temp_wav:
            ...
            # Step 3: Transcribe the audio segment using SpeechRecognition
            recognizer = sr.Recognizer()

            with sr.AudioFile(temp_wav.name) as source:
                audio_data = recognizer.record(source)

            # Use Google Web Speech API (can be replaced with other services if desired)
            transcription = recognizer.recognize_google(audio_data)

        return transcription

    except Exception as e:
        return f"Error transcribing the YouTube segment: {str(e)}"
...
async def generate_duckdb_query(query_type: str, timestamp_filter: str = None, numeric_filter: str = None, sort_order: str = "ASC") -> str:
    """
    Generate a DuckDB SQL query based on given parameters.

    Args:
        query_type (str): The type of query (e.g., "SELECT_ALL", "COUNT", etc.).
        timestamp_filter (str, optional): A timestamp condition (e.g., "timestamp > '2024-01-01'").
        numeric_filter (str, optional): A numeric condition (e.g., "price > 100").
        sort_order (str, optional): Sort order, either "ASC" or "DESC" (default is "ASC").

    Returns:
        str: The generated DuckDB SQL query.
    """
    ...
    conditions = []
    if timestamp_filter:
        conditions.append(timestamp_filter)
    if numeric_filter:
        conditions.append(numeric_filter)

    if conditions:
        base_query += " WHERE " + " AND ".join(conditions)

    # Add sorting
    base_query += f" ORDER BY id {sort_order}"  # Assuming sorting is by 'id'

    # Modify based on query type
    if query_type.upper() == "COUNT":
